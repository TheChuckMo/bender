...
@jira_cluster.group('upgrade')
@click.pass_context
def jira_cluster_upgrade(ctx):
...
@jira_cluster_upgrade.command('start')
@click.confirmation_option(prompt='Start a cluster upgrade?')
@click.pass_context
def jira_cluster_upgrade_start(ctx):
...
@jira_cluster_upgrade.command('approve')
@click.confirmation_option(prompt='Approve running upgrade?')
@click.pass_context
def jira_cluster_upgrade_approve(ctx):
...
@jira_cluster_upgrade.command('cancel')
@click.confirmation_option(prompt='Cancel running upgrade?')
@click.pass_context
def jira_cluster_upgrade_cancel(ctx):
...
@jira_cluster_upgrade.command('retry')
@click.confirmation_option(prompt='Retry the upgrade?')
@click.pass_context
def jira_cluster_upgrade_retry(ctx):
    """Retry upgrade steps"""
...
@jira_cluster.group('node')
@click.pass_context
def jira_cluster_node(ctx):
    """Jira cluster nodes.
...
@jira_cluster_node.command('state')
@click.pass_context
def jira_cluster_node_state(ctx):
    """Jira cluster nodes.
...
@jira_cluster_node.command('offline')
...
@click.pass_context
def jira_cluster_node_offline(ctx, nodeid):
